...
public class Utils {
	...
	public static String getPageBarHtml(int cPage, int numPerPage, int totalContents, String url) {
		...
		if(pageNo == 1) {
			...
			pageBar += "<li><a href='"+url+"cPage="+(pageNo-1)+"'><i class='pe-7s-angle-left'></i></a></li>"; //6페이지면 5페이지로 가게 //11페이지면 10페이지로 가게
		...
		while(pageNo <= pageEnd && pageNo <= totalPage ) {
			//현재페이지인 경우
			if(pageNo == cPage) {
				pageBar += "<li><a href='javascript:void(0)'>"+pageNo+"</a></li>";
			}else {
				pageBar += "<li><a href='"+url+"cPage="+pageNo+"'>"+pageNo+"</a></li>";
			}
			pageNo++;
		}
		//[다음]영역
		if(pageNo > totalPage) {
			...
			//totalpAGE보다 큰경우
			pageBar += "<li><a href='"+url+"cPage="+pageNo+"'><i class='pe-7s-angle-right'></i></a></li>"; //6페이지면 5페이지로 가게 //11페이지면 10페이지로 가게
		}
